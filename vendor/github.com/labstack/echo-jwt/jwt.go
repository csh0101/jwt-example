...
func (config Config) ToMiddleware() (echo.MiddlewareFunc, error) {
	...
	return func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			fmt.Println("xxxxxxxxxxxxxxxxxxxxxxxxxxxx111111111")
			if config.Skipper(c) {
				return next(c)
			}
			...
			fmt.Println("xxxxxxxxxxxxxxxxxxxxxxxxxxxx2222222")
			...
			for _, extractor := range extractors {
				...
				for _, auth := range auths {
					...
					fmt.Println(token)
					fmt.Println(config.ContextKey)
					fmt.Println("ggggxxxxxxxxxxxx")
					return next(c)
				}
			}
			...
			err = lastTokenErr
			fmt.Println("gggg" + lastTokenErr.Error())
			if err == nil {
